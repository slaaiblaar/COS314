...
void howdyNeighbours(int *sol, int neighbours[][numVecs])
{
    ...
    for (int n = 0; n < NUM_NEIGHBOURS; ++n)
    {
        ...
        do
        {
            ...
            v2 = (rand() % (numVecs - 1)) + 1;
#ifdef DEBUG
            std::cout << "NUM_NEIGHBOURS: " << NUM_NEIGHBOURS << ", n: " << n << std::endl;
            std::cout << "Random V1: " << v1 << ", V2: " << v2 << std::endl;
#endif
        ...
        for (int v = 0; v < numVecs; ++v)
        {
            std::cout << labels[neighbours[n][v]] << "->";
        }
...
void generateInitialSol(int *sol)
{
    ...
    for (int n = numVecs - 1; n > 0; --n)
    {
        randCampus = rand() % n;
#ifdef DEBUG
        std::cout << "RandCampus: " << randCampus << std::endl;
#endif
        int counter = 0;
        while (randCampus >= 0 && counter < numVecs)
        {
            if (!visited[counter])
            {
                if (randCampus == 0)
                    break;
#ifdef DEBUG
                std::cout << counter << " not visited, decrementing randCampus to " << --randCampus << std::endl;
#else
                --randCampus;
#endif
            }
...
#ifdef DEBUG
        std::cout << "Selected random campus: " << counter << std::endl;
#endif
